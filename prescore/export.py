...
from . import clock, config, publish, settings, store
...
def write(
    ...
    write_config(model_version)
    return target


def write_config(model_version: str | None = config.MODEL_VERSION, path: Path | None = None) -> Path:
    """web/config.js: what the live pages need to query Supabase directly.

    The anon key is meant to be shipped in client-side JS -- Row Level
    Security enforces what it can read and write, not the secrecy of this
    string (see supabase/migrations/). Regenerated on every export, same as
    data.json, so a model version bump reaches the live pages automatically
    without a separate step to remember.
    """
    target = path or CONFIG_PATH
    target.parent.mkdir(parents=True, exist_ok=True)

    url = settings.get("SUPABASE_URL") or ""
    anon_key = settings.get("SUPABASE_ANON_KEY") or ""
    ...
    target.write_text(
        ...
        f"  modelVersion: {json.dumps(model_version)},\n"
        f"  league: \"EPL\",\n"
        "};\n",
        encoding="utf-8",
    )
